          ...
          {selectedList.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-1.5 items-center">
              {selectedList.slice(0, 10).map((p) => (
                <span
                  key={p.id}
                  className={`text-xs px-2.5 py<dyad-write path="src/components/PrerequisitesBar.tsx" description="Prerequisites summary bar and expandable selector modal">
import React, { useState } from 'react';
import { Prerequisite } from '../types';
import { Search, X, Check } from 'lucide-react';
import { normalize } from '../lib/utils';
...
export const PrerequisitesBar: React.FC<PrerequisitesBarProps> = ({
  ...
  return (
    <div className="mb-8">
      ...
      {isOpen && !locked && (
        <div className="mt-3 bg-surface border border-border rounded-xl shadow-md overflow-hidden animate-in fade-in duration-150">
                  ...
                  <div key={corde} className="space-y-3">
                    ...
                    <div className="space-y-3 pl-1">
                      {Object.entries(byCategory).map(([cat, items]) => (
                        <div key={cat}>
                          <span className="block text-[11px] font-semibold text-muted uppercase tracking-wider mb<dyad-write path="src/components/PrerequisitesBar.tsx" description="Prerequisites summary bar and expandable selector modal">
import React, { useState } from 'react';
import { Prerequisite } from '../types';
import { Search, X, Check } from 'lucide-react';
import { normalize } from '../lib/utils';
